...
parser.add_argument("--household_density", type=float, default=0.9,
                    help="Total fraction of grid cells occupied by households")
parser.add_argument("--student_density", type=float, default=1,
                    help="Average amount of students per household")
parser.add_argument("--max_move_fraction", type=float, default=0.25,
                    help="Maximum fraction of agents allowed to move during a full step")
parser.add_argument("--max_res_steps", type=int, default=0,
                    help="Maximum amount of steps during residential process")
parser.add_argument("--max_school_steps", type=int, default=300,
                    help="Maximum amount of steps during school process")
parser.add_argument("--conv_threshold", type=float, default=0.01,
                    help="Convergence threshold for the residential and school processes")

# Spatial object parameters
parser.add_argument("--n_neighbourhoods", type=int, default=16,
                    help="Number of neighbourhood objects to be placed")
parser.add_argument("--neighbourhoods_placement", type=str, default="evenly_spaced",
                    help="Placement method of neighbourhoods: random, evenly_spaced")
parser.add_argument("--bounded_neighbourhood", action="store_true", default=False,
                    help="Use bounded neighbourhood model if true, unbounded if omitted")
parser.add_argument("--n_schools", type=int, default=16,
                    help="Amount of schools to be place in grid")
parser.add_argument("--schools_placement", type=str, default="evenly_spaced",
                    help="Placement method of schools: random, evenly_spaced, random_per_neighbourhood")

# Household parameters
parser.add_argument("--group_categories", type=str, nargs="+", default=["etnicity"],
                    help="Space seperated list of categories")
parser.add_argument("--group_types", type=str, nargs="+", default=[["a", "b"]],
                    help="Space seperated list of strings containing comma seperated list of types within each group")
parser.add_argument("--group_dist", type=str, nargs="+", default=[[0.5, 0.5]],
                    help="Space seperated list of strings containing comma seperated list of density of subtype")
parser.add_argument("--category_thresholds", type=str, nargs="+", default=[[1, 1]],
                    help="Space seperated list of strings containing comma seperated list of density of subtype")
parser.add_argument("--utility_at_max", type=str, nargs="+", default=[[0.6, 0.6]],
                    help="Space seperated list of strings containing comma seperated list of density of subtype")
parser.add_argument("--optimal_fraction", type=str, nargs="+", default=[[0.5, 0.5]],
                    help="Space seperated list of strings containing comma seperated list of density of subtype")

# School parameters
parser.add_argument("--school_capacity", type=float, default=2,
...
parser.add_argument("--moore", dest="moore", action="store_true",
                    help="Use Moore neighbours when finding neighbours")
parser.add_argument("--no-moore", dest="moore", action="store_false",
                    help="Do not use Moore neighbours when finding neighbours")
parser.set_defaults(moore=True)
parser.add_argument("--neighbourhood_mixture", type=float, default=0.2,
                    help="Ratio bounded neighbourhood to local neighbourhood used in Shelling model. \
                            Zero equals fully local, one fully bounded neighbourhood.")

# Other parameters
parser.add_argument("--alpha", type=float, default=0.2,
                    help="School utility parameter in Cobb-Douglas utility function")
parser.add_argument("--filename", type=str, default='data/data',
                    help="Filename to save the data.")
parser.add_argument("--loglevel", type=str, default='ERROR',
                    help="Level for the logger to use.")
...
parser.add_argument("--case", type=str, default='IJburg',
                    help="To implement case studies. Lattice, Amsterdam or IJburg are \
                        available for now.")
parser.add_argument("--random_residential", type=int, default=0,
                    help="To generate random residential patterns.")
parser.add_argument("--logging_enabled", type=bool, default=False,
                    help="To enable logging or not.")
...
if type(FLAGS.group_types[0]) == str:
    ...
    FLAGS.optimal_fraction = [elem.split(",") for elem in FLAGS.optimal_fraction]

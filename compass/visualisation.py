...
class BokehServer():
    ...
    def composition_data(self, agent_type='school'):
        ...
        count = 0
        data = self.data[self.data.agent_type==agent_type]
        reds = [0]*data.shape[0]
        blues = [0]*data.shape[0]

        for index, row in data.iterrows():

            total = row.group0 + row.group1

            if total==0:
                total = 1

            blues[count] = row.group0 / total
            reds[count] = row.group1 / total
            count += 1

        composition_data = {'blues':blues, 'reds':reds}
    ...
    def update_data(self):
        ...
        for quads in [self.neighbourhood_composition_quads, self.school_composition_quads]:
    ...
    def layout(self):
        ...
        third_col = column([self.plot, self.distribution_plot, 
            self.neighbourhood_composition_plot, self.school_composition_plot])
    ...
    def widgets(self, width=100):
        ...
        self.case = Select(title='Case', 
            options=['Amsterdam', 'IJburg', 'Lattice'],
        ...
        self.size = Select(title="Size",
            options=[str(x * 10) for x in range(1, 11)],
        ...
        if self.params['case'].lower() != 'lattice':
            ...
            self.optimal_fraction, self.ranking_method]

...
class CompassModel(Model):
    ...
    def __init__(self, params, export=False):
        ...
        self.global_composition_normalized = (
            self.global_composition / self.global_composition.sum()
        )
    ...
    def set_agent_parameters(self, params, households):
        ...
        optimal_fractions = self.trunc_normal_sample(
            params["optimal_fraction"][0], scale=params["homophily_std"], size=n_agents
        )
        ...
        utility_at_maxs = self.trunc_normal_sample(
            params["utility_at_max"][0], scale=params["homophily_std"], size=n_agents
        )
        ...
        for household in households:
            ...
            self.optimal_fraction[array_index] = optimal_fractions[household.category][
                array_index
            ]
            self.alpha[array_index] = alphas[household.category][array_index]
            self.utility_at_max[array_index] = utility_at_maxs[household.category][
                array_index
            ]
    ...
    def calc_school_compositions(self):
        ...
        for household in self.get_agents("households"):
            ...
            self.school_compositions[array_index] = household.students[
                0
            ].school.composition_normalized[category]
    ...
    def calc_school_rankings(self, households, schools):
        ...
        # utilities = (
        #     composition_utilities * self.alpha[np.newaxis, :]
        #     + (self.distance_utilities * (1 - self.alpha[:, np.newaxis])).T
        # )
        utilities = self.distance_utilities.T
    ...
    def compute_school_distances(self):
        ...
        for household in self.get_agents("households"):
            self.all_distances[household.array_index, :] = school_frame.distance(
                # # TODO: different position of household is used here
                # household.shape
                Point(household.pos)
            )

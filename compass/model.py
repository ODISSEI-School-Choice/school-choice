...
class CompassModel(Model):
    ...
    Attributes:
        ...
        agents (dict of []): agents in the model, by type: amount, households, schools, neighbourhoods
    ...
    def __init__(self, params, export=False):
        ...
        self.global_composition_normalized = self.global_composition / self.global_composition.sum()
    ...
    def set_attributes(self, params, export=False):
        ...
        params['n_students'] = int(
                params["n_households"] * params["student_density"]
                )
    ...
    def set_agent_parameters(self, params, households):
        ...
        Calculates (constant during run):
            categories ([int]) array of the household.category values

        Allocates (changes over run):
            distances  ([float]) array of the distance of the household to school of the first student
        ...
        self.local_compositions = np.zeros(n_agents, dtype=dtype)
        self.neighbourhood_compositions = np.zeros(n_agents, dtype=dtype)
        ...
        optimal_fractions = self.trunc_normal_sample(
                                ...
                                size=n_agents)
        alphas = self.trunc_normal_sample(
        ...
        for array_index, household in enumerate(households):
            ...
            self.optimal_fraction[array_index] = optimal_fractions[household.category][array_index]
            self.alpha[array_index] = alphas[household.category][array_index]
            self.utility_at_max[array_index] = utility_at_maxs[household.category][array_index]
    ...
    def create_households(self):
        ...
        for i, position in enumerate(empties[0:params["n_households"]]):
            ...
            self.household_attrs[
                    position[0], position[1], :
                    ] = household.attributes

        # Calculate AFTER all agents are placed
        all_households = self.agents['households']
    ...
    def create_households(self, household_frame, actual_households):
        ...
        for index, row in households.iterrows():
            self.create_single_household(index, row, n_agents, neighbourhoods)

        self.location_to_agent()

    def create_single_household(self, index, row, n_agents, neighbourhoods):
        """
    ...
    def location_to_agent(self):
        ...
        self.location_to_agent = {
                str(agent.pos): agent for agent in agents
                }
    ...
    def calc_residential_compositions(self):
        ...
        compositions = np.zeros(
                shape=self.household_attrs.shape,
                dtype="float32"
                )
    ...
    def calc_school_rankings(self, households, schools):
        ...
        composition_utilities = np.where(
                x <= t,
                x / t,
                M + (1-x)*(1-M) / (1-t)
                )
        ...
        utilities = composition_utilities * self.alpha[np.newaxis, :] + (
                self.distance_utilities * (1 - self.alpha[:, np.newaxis])
                ).T
        ...
        households_utilities = np.fromiter(
                [h.utility for h in households],
                dtype="float32"
                )
    ...
    def simulate(self, res_steps=None, school_steps=None):
        ...
        while (
                self.scheduler.get_time('residential') < res_steps
                and not self.res_ended
                ):

            if self.verbose:
                f = "Residential process: step " \
        ...
        while (
                self.scheduler.get_time('school') < school_steps
                and not self.school_ended
                ):

            if self.verbose:
                f = "School process: step " \
    ...
    def convergence_check(self):
        ...
        if time >= window_size-1:
            ...
            metric_means = np.repeat(
                    metrics.mean(axis=1)[:, np.newaxis],
                    window_size,
                    axis=1
                    )
    ...
    def choose_locations(self, amount, method="evenly_spaced"):
        """
        Compute a number of locations to place school and neighbourhood
        objects.  Currently, only random and evenly spaced locations are
        allowed.
            ...
            for y in range(self.params["height"]):
                ...
                for neighbourhood in neighbourhoods:
                    ...
                    if shape.contains(point):
                        closest_neighbourhoods[str((x, y))] = str(
                                neighbourhood.pos
                                )
                        break

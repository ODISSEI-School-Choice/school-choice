...
def log_segregation(model):
    ...
    school_seg = model.measurements.calculate_segregation(type="school", index="Theil")
...
def random_params():
    ...
    params["n_neighbourhoods"] = np.random.choice([x**2 for x in range(3, 8)])
    params["n_schools"] = np.random.choice([x**2 for x in range(3, 8)])
...
def test_distance_only(random_params):
    ...
    # TODO: too large temperature that results in inf utilities and cause value error of random.choice
